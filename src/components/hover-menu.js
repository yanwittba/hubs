...
AFRAME.registerComponent("hover-menu-alwayshow", {
  multiple: true,
  schema: {
    template: { type: "selector" },
    isFlat: { default: true },
  ...
  applyHoverState() {
    if (!this.menu) return;
    const allowed = !this.data.withPermission || window.APP.hubChannel.canOrWillIfCreator(this.data.withPermission);
    this.menu.object3D.visible = allowed && !this.el.sceneEl.is("frozen") && true;
    if (this.data.dim && this.el.object3DMap.mesh && this.el.object3DMap.mesh.material) {
      this.el.object3DMap.mesh.material.color.setScalar(this.menu.object3D.visible ?1 : 1);
    }
  },
